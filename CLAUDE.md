...
**Never convert real output to a demo.** Footage of the *editor's UI* may become an in-page demo — a recreation shows the same thing more directly and drops 1–2 MB of third-party JS. Footage of *real finished work* may not: the five shorts in `WhatIsFlashFX` are actual output made with FlashFX and stay as video (owner's decision, 2026-08-07). Showing a simulation where a visitor believes they're seeing genuine results is a different claim from illustrating an interface. **The embed count is not a number to drive to zero** — when trimming embeds, ask what each is evidence *of*.

**Live demos**: `components/demos/` (I3) replaced seven YouTube embeds with in-page recreations of the editor — keyframe timeline, NLE clip timeline, 3D cube, easing curve, preset wall, share. **`TimelineDemo` and `ClipTimelineDemo` are deliberately different instruments** (keyframes on property tracks vs. clips in a sequence); they were the same component in two sections until 2026-08-07 and must not be merged back. A demo declares its own card width and aspect in `demoFrame` — the default 16:9 `max-w-5xl` is a video's shape and wrong for a timeline. Add one by giving `VideoPlaceholder` a `demo` prop (`DemoKind`), which takes precedence over `youtubeId`; the ids are left in place so reverting a section is a one-word edit. Every demo is `dynamic({ ssr: false })` via `components/demos/index.tsx` — **register new ones there, not with a direct import**, or they land in the initial bundle. Each holds a governor slot at `DEMO_PRIORITY` (above decoration) and must render a composed still frame when `active` is false, never a blank.
...
**Four** fonts are loaded in `app/layout.tsx` via `next/font/google` and exposed as CSS variables: `--font-cormorant` (display, `font-display`), `--font-outfit` (`font-sans`, body and UI), `--font-jetbrains` (`font-mono`), and `--font-inter` — one weight, added 2026-08-07 for the two big statement headings (`SolutionSection`, `OrganizeWorkflow`). Inter is a deliberate step back from P7's three-family target; it is `preload: false`, so the single preload slot still belongs to Outfit. Existing components mostly apply these through inline `style={{ fontFamily: 'var(--font-…)' }}` rather than the Tailwind font classes — match whichever the surrounding file uses.
